...
// ==================================================================
const WorkflowEventTrigger = types
  .model('WorkflowEventTrigger', {
    ...
    targetIds: types.optional(types.array(types.string), []),
  })
  .actions(self => ({
    setWorkflowEventTrigger(eventTrigger) {
      applySnapshot(self, eventTrigger);
    },
  }))
...
const Workflow = types
  .model('Workflow', {
    ...
    eventTriggers: types.optional(types.array(WorkflowEventTrigger), []),
  })
  .actions(self => ({
    ...
    setEventTriggers(eventTriggers) {
      // we try to preserve any existing event trigger objects and update their content instead
      const mapOfExisting = _.keyBy(self.eventTriggers, 'id');
      const processed = [];

      _.forEach(eventTriggers, eventTrigger => {
        const existing = mapOfExisting[eventTrigger.id];
        if (existing) {
          existing.setWorkflowEventTrigger(eventTrigger);
          processed.push(existing);
        } else {
          processed.push(WorkflowEventTrigger.create(eventTrigger));
        }
      });

      self.eventTriggers.replace(processed);
    },

    deleteEventTrigger(eventTriggerToDelete) {
      const processed = [];

      _.forEach(self.eventTriggers, eventTrigger => {
        if (eventTrigger.id !== eventTriggerToDelete.id) {
          processed.push(eventTrigger);
        }
      });
    ...
    addEventTrigger(eventTriggerToAdd) {
      const processed = self.eventTriggers.toJS();
      processed.push(eventTriggerToAdd);
      self.eventTriggers.replace(processed);
    },
  }))
