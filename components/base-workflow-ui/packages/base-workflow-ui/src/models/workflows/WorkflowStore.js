...
import WorkflowEventTriggersStore from './WorkflowEventTriggersStore';
...
const WorkflowStore = BaseStore.named('WorkflowStore')
  .props({
    ...
    eventTriggersStore: types.optional(types.map(WorkflowEventTriggersStore), {}),
  ...
  .actions(self => {
    ...
    return {
      ...
      getEventTriggersStore: () => {
        const workflowId = self.workflowId;
        let entry = self.eventTriggersStore.get(workflowId);
        if (!entry) {
          // Lazily create a WorkflowEventTriggersStore for each workflow
          self.eventTriggersStore.set(workflowId, WorkflowEventTriggersStore.create({ workflowId }));
          entry = self.eventTriggersStore.get(workflowId);
        }
