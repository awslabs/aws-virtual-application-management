...
import WorkflowPublishedList from '../WorkflowPublishedList';

jest.mock('../../../../images/white-gradient.png');
jest.mock('../../../workflow-common/ProgressPlaceholder');

describe('WorkflowPublishedList', () => {
  ...
  it('WorkflowPublishedList should renders correctly', () => {
    expect(
      renderer
        .create(
          <Provider workflowsStore={workflowsStore} pluginRegistry={pluginRegistry}>
            <WorkflowPublishedList.WrappedComponent workflowsStore={workflowsStore} userDisplayName={userDisplayName} />
          </Provider>,
        )
        .toJSON(),
    ).toMatchSnapshot();
