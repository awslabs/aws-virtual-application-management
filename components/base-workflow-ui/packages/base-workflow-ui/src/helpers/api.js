...
async function getWorkflowEventTriggers(id) {
  return httpApiGet(`api/workflows/${encodeURIComponent(id)}/event-triggers`);
}

async function deleteWorkflowEventTrigger(workflowId, triggerId) {
  return httpApiDelete(
    `api/workflows/${encodeURIComponent(workflowId)}/event-triggers/${encodeURIComponent(triggerId)}`,
  );
}

async function createWorkflowEventTrigger(workflowId, eventTrigger) {
  return httpApiPost(`api/workflows/${encodeURIComponent(workflowId)}/event-triggers`, { data: eventTrigger });
}
