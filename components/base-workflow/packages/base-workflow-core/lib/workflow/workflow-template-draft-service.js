...
class WorkflowTemplateDraftService extends Service {
  ...
  async createDraft(
    ...
    if (isNewTemplate) {
      const existing = await workflowTemplateService.findVersion(requestContext, { id: templateId });
      ...
      draft.template = await workflowTemplateService.mustFindVersion(requestContext, {
        id: templateId,
        v: templateVer,
      });
    }
  ...
  async updateDraft(requestContext, draft = {}) {
    ...
    if (template.id !== originalTemplate.id || template.v !== originalTemplate.v) {
      originalTemplate = await workflowTemplateService.mustFindVersion(requestContext, {
        id: template.id,
        v: template.v,
      });
    }
  ...
  async publishDraft(requestContext, draft = {}) {
    ...
    const existing = await workflowTemplateService.findVersion(requestContext, { id: template.id });
    let newVersion = 1;
