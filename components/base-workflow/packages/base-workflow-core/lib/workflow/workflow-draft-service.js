...
import { ensureAdmin, runAndCatch } from '@aws-ee/base-services';
...
class WorkflowDraftService extends Service {
  ...
  async createDraft(
    ...
    if (isNewWorkflow) {
      ...
      const existingWorkflow = await workflowService.findVersion(requestContext, { id: workflowId });
      ...
      const template = await workflowTemplateService.mustFindVersion(requestContext, {
        id: templateId,
        v: templateVer,
      });
      ...
      draft.workflow = await workflowService.prepareWorkflow(requestContext, draft.workflow);
    } else {
  ...
  async updateDraft(requestContext, draft = {}) {
    ...
    const originalDraft = await this.mustFindDraft(requestContext, { id: draft.id });
    ...
    const preparedWorkflow = await workflowService.prepareWorkflow(requestContext, mergedWorkflow);
  ...
  async publishDraft(requestContext, draft = {}) {
    ...
    const existing = await workflowService.findVersion(requestContext, { id: workflow.id });
    let newVersion = 1;
  ...
  async deleteDraft(requestContext, { id }) {
    ...
    const originalDraft = await this.mustFindDraft(requestContext, { id });
  ...
  async findDraft(requestContext, { id, fields = [] }) {
    await ensureAdmin(requestContext);
    const dbService = await this.service('dbService');
    const table = this.tableName;
  ...
  async mustFindDraft(requestContext, { id, fields }) {
    await ensureAdmin(requestContext);
    const draft = await this.findDraft(requestContext, { id, fields });
