...
class StepTemplateService extends Service {
  ...
  async updateVersion(requestContext, manifest = {}, { isLatest = true, tableName } = {}) {
    ...
    const result = await runAndCatch(
      ...
      async () => {
        ...
        const existing = await this.findVersion(requestContext, { id, v, fields: ['id', 'v'] });
        if (existing) {
  ...
  async mustFindVersion(requestContext, { id, v = 0, fields }) {
    ...
    const step = await this.findVersion(requestContext, { id, v, fields });
    if (!step) throw this.boom.notFound(`The step template "${id}" ver "${v}" is not found`, true);
    return step;
  }

  async mustValidateVersion(requestContext, { id, v = 0, config = {} }) {
    await ensureAdmin(requestContext);

    const [inputManifestValidationService] = await this.service(['inputManifestValidationService']);
    const step = await this.mustFindVersion(requestContext, { id, v });
