...
class WorkflowTemplateService extends Service {
  ...
  async createVersion(requestContext, manifest = {}, { isLatest = true, tableName } = {}) {
    ...
    manifest = await this.populateSteps(manifest, requestContext);
  ...
  async mustFindVersion(requestContext, { id, v = 0, fields }) {
    const workflowTemplate = await this.findVersion(requestContext, { id, v, fields });
  ...
  async populateSteps(manifest, requestContext) {
