...
import { Menu, Icon, Image, Button, Modal } from 'semantic-ui-react';
...
class MainLayout extends React.Component {
  ...
  renderReleaseDetails() {
    ...
    return (
      <Menu.Item style={{ backgroundColor: shouldRenderDisclaimer && 'rgb(126, 12, 12)', maxWidth: '400px' }}>
        <div style={{ marginRight: shouldRenderDisclaimer && '10px' }}>
          {version}: {versionDisclaimerHeader}
        </div>
        {shouldRenderDisclaimer && this.renderReleaseDisclaimerModal()}
      </Menu.Item>
    );
  }

  renderReleaseDisclaimerModal() {
    return (
      <Modal
        centered={false}
        style={{ marginTop: '100px' }}
        trigger={
          <Button compact basic color="yellow" circular icon style={{ display: 'contents' }}>
            <Icon name="question circle" style={{ paddingBottom: '15px' }} />
          </Button>
        }
        closeIcon
        header={versionDisclaimerHeader}
        content={versionDisclaimerContent}
      />
    );
  }
}
