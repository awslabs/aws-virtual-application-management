...
class CapabilitiesList extends React.Component {
  ...
  componentDidMount() {
    const store = this.userCapabilitiesStore;
  ...
  get userCapabilitiesStore() {
    const store = this.props.userCapabilitiesStore;
    return store;
  }
  ...
  renderMain() {
    const userCapabilityData = this.userCapabilities;
    const pageSize = userCapabilityData.length;
    const showPagination = userCapabilityData.length > pageSize;
    return (
      <div>
        <ReactTable
          data={userCapabilityData}
  ...
  renderTotal() {
    return <Label circular>{this.userCapabilities.length}</Label>;
  }
  ...
  render() {
    ...
    return (
      <Container className="mt3 animated fadeIn">
        <DocumentationClient urlSuffix="user_guide/sidebar/admin/users/user_capabilities" />
        {this.renderHeader()}
        {content}
      </Container>
    );
...
decorate(CapabilitiesList, {
  mapOfUsersBeingEdited: observable,
  formProcessing: observable,
});

export default inject('userCapabilitiesStore')(withRouter(observer(CapabilitiesList)));
