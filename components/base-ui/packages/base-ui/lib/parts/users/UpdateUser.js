...
class UpdateUser extends React.Component {
  constructor(props) {
    super(props);
    runInAction(() => {
      this.stores = new Stores([this.userStore, this.userRolesStore, this.authenticationProviderConfigsStore]);
      this.modalOpen = false;
      this.processing = false;
      this.view = 'detail'; // view mode or edit mode
    });
    this.form = getUpdateUserConfigForm(this.currentUser);
  }
  ...
  renderDetailView() {
    ...
    const toRow = fieldName => {
      const value = _.get(this.currentUser, fieldName);
  ...
  renderEditView() {
    ...
    const identityProviderOptions = this.identityProviderOptions;
    const userRoleOptions = this.userRoleOptions;
  ...
  handleFormSubmission = async form => {
    ...
    let userToUpdate = { ..._.omit(this.currentUser, 'userCapabilities'), firstName, lastName, email };
    if (this.props.adminMode) {
      userToUpdate = { ...userToUpdate, userRole, isAdmin, status };
    }
    ...
    try {
      ...
      if (identityProviderNameField.isDirty) {
        ...
        if (this.props.adminMode) {
          ...
          await usersStore.deleteUser(this.currentUser);
        } else {
  ...
  handleDeleteClick = async () => {
    try {
      ...
      await this.usersStore.deleteUser(_.omit(this.currentUser, 'userCapabilities'));
    } catch (error) {
      displayError(error);
    }
    runInAction(() => {
  ...
  handleApproveDisapproveClick = async status => {
    try {
      this.processing = true;
      await this.usersStore.updateUser({ ..._.omit(this.currentUser, 'userCapabilities'), status });
  ...
  handleOpen = () => {
    ...
    // Need to recreate form based on the user being passed (i.e., currentUser) to make sure the form field values
