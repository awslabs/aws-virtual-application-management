...
const AuthenticationProviderConfigsStore = BaseStore.named('AuthenticationProviderConfigsStore')
  ...
  .views(self => ({
    ...
    get cognitoAuthProvider() {
      // Return the first provider config whose ID points at a Cognito user pool endpoint
      const userPoolRegex = '^https://cognito-idp\\.[-a-z0-9]+\\.amazonaws\\.com/([-a-z0-9]+_[A-Za-z0-9]+)$';
      return _.find(self.list, providerConfig => !_.isNil(providerConfig.id.match(userPoolRegex)));
    },
