...
function adjustRedirectUri(uri, redirectType = 'login') {
  ...
  if (initialRedirectUri !== websiteUrl) {
    ...
    adjustedUri = addQueryParams(adjustedUri, {
      [redirectParamName]: window.location.origin,
    });
  }

  // Add current location and query params to state if we want the user to return to this page after logging in
  if (preservePath) {
    const currLocation = {
    ...
    adjustedUri = addQueryParams(adjustedUri, { state });
  }
      ...
      try {
        ...
        if (self.credentialHandlingType === 'submit') {
          ...
          throw boom.incorrectImplementation(
            'This application has not been configured properly. Native login without redirect is not implemented.',
          );
