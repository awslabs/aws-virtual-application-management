...
const Authentication = types
  ...
  .actions(self => ({
    ...
    handleCompletedLogin() {
      const { idToken, decodedIdToken } = self.getIdTokenInfo();

      // Set selected authentication provider. This is used during logout
      self.setSelectedAuthenticationProviderId(decodedIdToken.iss);
