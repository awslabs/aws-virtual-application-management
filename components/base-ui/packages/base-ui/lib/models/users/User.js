...
const User = types
  .model('User', {
    ...
    status: 'active',
    userRole: 'guest',
    isExternalUser: types.optional(types.boolean, false),
    rev: 0,
    userCapabilities: types.optional(types.array(types.string), []),
  })
  .views(self => ({
    get displayName() {
    ...
    get capabilities() {
      ...
      toJS(self.userCapabilities).forEach(capabilityId => {
        capabilitiesMap[capabilityId] = true;
      });
