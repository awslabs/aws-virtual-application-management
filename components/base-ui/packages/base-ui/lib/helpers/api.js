...
function setApiPaths(paths) {
  apiPaths = _.map(paths, ({ route, apiPath: ap }) => ({ route, regexp: pathToRegexp(route), apiPath: ap }));
}
...
function getApiPath(urlPath) {
  const matchingApiPath = _.find(apiPaths, ({ regexp }) => regexp.test(urlPath));

  if (matchingApiPath) {
    return matchingApiPath.apiPath;
  }

  // Default to the settings apiPath
...
function httpApiGet(urlPath, { params } = {}) {
  return fetchJson(`${getApiPath(urlPath)}/${urlPath}`, {
    method: 'GET',
...
function getUserCapabilities() {
  return httpApiGet('api/user-capabilities');
}
