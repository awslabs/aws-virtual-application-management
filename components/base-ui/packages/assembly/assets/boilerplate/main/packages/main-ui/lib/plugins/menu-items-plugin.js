 ...
 * This function is called last after adding navigation menu items to the itemsMap from all other installed components.
 *
...
function registerMenuItems(itemsMap, { location, appContext }) {
  ...
  const canViewHelloWorldMessages = _.get(appContext, 'userStore.user.capabilities.canViewHelloWorldMessages');

  const items = new Map([
    ...itemsMap,
    ['/hello', { title: 'Hello', icon: 'hand spock', shouldShow: canViewHelloWorldMessages }],
  ]);
