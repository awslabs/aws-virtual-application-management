...
class CreateCloudFrontInterceptor extends Service {
  ...
  async cleanup() {
    await this.removeCfLambdaAssociations();
  }

  async removeCfLambdaAssociations() {
    const aws = await this.service('aws');
    const cloudFrontApi = new aws.sdk.CloudFront({ apiVersion: '2019-03-26' });
    const cloudFrontId = this.settings.get('cloudFrontId');
    const existingConfig = await cloudFrontApi.getDistributionConfig({ Id: cloudFrontId }).promise();

    const countDefaultAssociations = _.get(
      existingConfig,
      'DistributionConfig.DefaultCacheBehavior.LambdaFunctionAssociations.Quantity',
    );
    const cacheBehaviors = _.get(existingConfig, 'DistributionConfig.CacheBehaviors.Items', []);
    const countAssociations = _.sum(_.map(cacheBehaviors, cb => _.get(cb, 'LambdaFunctionAssociations.Quantity', 0)));

    if (countDefaultAssociations > 0 || countAssociations > 0) {
      // Lambda@Edge are associated with one or more cache behaviors of the CloudFront distribution
      // The distribution requires update to clear out the associations
      this.log.info('Edge Lambda Associations are present; removing them now.');
    } else {
      this.log.info('Edge Lambda Associations are NOT present; nothing to do.');
      return undefined;
    }
    ...
    const newCloudFrontConfig = {
      ...existingConfig,
      Id: cloudFrontId,
      IfMatch: existingConfig.ETag,
    };
    delete newCloudFrontConfig.ETag;

    // Clear out LambdaFunctionAssociations from DefaultCacheBehavior
    _.set(newCloudFrontConfig, 'DistributionConfig.DefaultCacheBehavior.LambdaFunctionAssociations.Quantity', 0);
    _.set(newCloudFrontConfig, 'DistributionConfig.DefaultCacheBehavior.LambdaFunctionAssociations.Items', []);

    // Clear out LambdaFunctionAssociations all other cache behaviors
    const newCacheBehaviors = _.get(newCloudFrontConfig, 'DistributionConfig.CacheBehaviors.Items', []);
    _.forEach(newCacheBehaviors, cb => {
      _.set(cb, 'LambdaFunctionAssociations.Quantity', 0);
      _.set(cb, 'LambdaFunctionAssociations.Items', []);
    });

    return cloudFrontApi.updateDistribution(newCloudFrontConfig).promise();
  }
}

export default CreateCloudFrontInterceptor;
