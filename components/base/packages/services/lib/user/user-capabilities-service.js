...
import createSchema from '../schema/create-user-capability.json';
import updateSchema from '../schema/update-user-capability.json';
...
class UserCapabilitiesService extends Service {
  constructor() {
    super();
  ...
  async mustFind(requestContext, { id, fields = [] }) {
    ...
    if (!result) throw this.boom.notFound(`user capability with id "${id}" does not exist`, true);
    return result;
  }

  async create(requestContext, userCapability) {
    // ensure that the caller has permissions to create the user capability
    ...
    await validationService.ensureValid(userCapability, createSchema);

    // For now, we assume that 'createdBy' and 'updatedBy' are always users and not groups
    const by = _.get(requestContext, 'principalIdentifier.uid');
    const { id } = userCapability;

    // Prepare the db object
    const dbObject = this._fromRawToDbObject(userCapability, {
      rev: 0,
      createdBy: by,
      updatedBy: by,
    });
    ...
    const result = await runAndCatch(
      ...
      async () => {
        throw this.boom.badRequest(`user capability with id "${id}" already exists`, true);
      },
    );

    // Write audit event
    await this.audit(requestContext, { action: 'create-user-capability', body: result });

    return result;
  }

  async update(requestContext, rawData) {
    // ensure that the caller has permissions to update the user capability information
    // Perform default condition checks to make sure the user is active and is admin
    await this.assertAuthorized(
      requestContext,
    ...
    const result = await runAndCatch(
      ...
      async () => {
        ...
        // 1 - The user capability does not exist
        ...
        if (existing) {
          throw this.boom.badRequest(
            `user capability information changed by "${existing.updatedBy}" just before your request is processed, please try again`,
            true,
          );
        }
        throw this.boom.notFound(`user capability with id "${id}" does not exist`, true);
      },
    );

    // Write audit event
    await this.audit(requestContext, { action: 'update-user-capability', body: result });

    return result;
  }

  async delete(requestContext, { id }) {
    // ensure that the caller has permissions to delete the user capability
    // Perform default condition checks to make sure the user is active and is admin
    await this.assertAuthorized(
      requestContext,
    ...
    // Write audit event
    await this.audit(requestContext, { action: 'delete-user-capability', body: { id } });
  ...
  async list(requestContext, { maxResults = 1000, nextToken, fields = [] } = {}) {
    ...
    return this._query()
      ...
      .key('entityType', UserRoleEntityTypeMap.CAPABILITY)
      .limit(maxResults)
      .projection(fields)
      .queryPage(nextToken);
  }
  ...
  async assertAuthorized(requestContext, { action, conditions }, ...args) {
    ...
    await authorizationService.assertAuthorized(
      requestContext,
      { extensionPoint: 'user-capability-management-authz', action, conditions },
      ...args,
    );
  }
}
