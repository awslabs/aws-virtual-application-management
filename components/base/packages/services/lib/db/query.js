...
import { encode, decode } from '../utils/base64-url';
...
class DbQuery {
  ...
  // Get a single page of results, by default from the beginning, or from the optionally specified query.start()
  async queryPage(nextToken) {
    if (!this.params.Limit) {
      throw new Error('DbQuery.queryPage() Limit was not set. Must specify a limit before performing a page scan');
    }

    try {
      if (!_.isUndefined(nextToken) && nextToken !== 'null') {
        this.start(JSON.parse(decode(nextToken)));
      }
    } catch (err) {
    ...
    if (!_.isUndefined(data.LastEvaluatedKey)) {
      newNextToken = encode(JSON.stringify(data.LastEvaluatedKey));
    }

    return { items: unmarshal(data.Items), nextToken: newNextToken };
  }
}
