...
class S3Service extends Service {
  ...
  async listTagsForS3ObjectVersion(bucket, key, objectVersionId) {
    const resp = await this.api
      ...
      .promise();
    return _.get(resp, 'TagSet') || [];
  }

  async addTagToS3ObjectVersion(bucket, key, objectVersionId, tag) {
    return this.api
      .putObjectTagging({
        Bucket: bucket,
        Key: key,
  ...
  async removeTagFromS3ObjectVersion(bucket, key, objectVersionId, tag) {
    // There is no S3 API call to remove a specific tag from an object. Instead we first remove all tags,
    // and then selectively re-attach them (minus the one we wish to remove)
    const existingTags = await this.listTagsForS3ObjectVersion(bucket, key, objectVersionId);
    const remainingTags = existingTags.filter(existingTag => !_.isEqual(existingTag, tag));
    await this.api
      .deleteObjectTagging({
        Bucket: bucket,
        Key: key,
  ...
  async getTextObject(bucket, key, encoding = 'utf-8') {
