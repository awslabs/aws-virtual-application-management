...
function getDocusaurusConfig(docusaurusConfigSoFar, { docsSiteRootPath }) {
  return {
    // Solution-specific configuration overrides go here
    ...docusaurusConfigSoFar,
    baseUrl: `${docsSiteRootPath}/docs/`,
  };
}
...
async function getConfiguration(configSoFar, _pluginRegistry, settings) {
  const updatedConfig = {
    ...configSoFar,
    pagesPaths: getPagesPaths(configSoFar.pagesPaths),
    staticFilesPaths: getStaticFilesPaths(configSoFar.staticFilesPaths),
    docusaurusConfig: getDocusaurusConfig(configSoFar.docusaurusConfig, settings),
