...
import { createAwsSdkClient } from './utils';
 ...
 * @returns {Promise<{*}>}
 */
export async function getCloudFormationCrossRegionValues(
  stage,
  awsProfile,
  targetAwsRegion,
  currentSettings,
  ...
  const settingsForExpansion = { ...currentSettings, stage };
  ...
  const results = await Promise.all(
    Object.entries(cloudFormationSettings).map(async ([stackSettingNameKey, cfVariables]) => {
      ...
      const settingsToMerge = cfVariables.reduce((previous, { settingName, outputKey }) => {
        ...
        return setting ? { ...previous, [settingName]: setting.OutputValue } : previous;
      }, {});
