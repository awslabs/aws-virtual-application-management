...
import { getAwsAccountInfo } from './aws-acc-context';
import { getCloudFormationCrossRegionValues } from './cloud-formation-cross-region-values';
...
const newFileLoader = (serverless, options = {}) => async filename => {
  const { missingFiles = 'warn', emptyFiles = 'warn', loadedFile = 'info' } = options;
  if (missingFiles && !fs.existsSync(filename)) {
    if (missingFiles === 'warn') {
      console.warn(`Missing settings file: ${filename}`); // eslint-disable-line no-console
    }
    return {};
  }
  try {
    const result = await serverless.yamlParser.parse(filename);
    if (loadedFile === 'info') {
      console.info(`Loaded settings file: ${filename}`); // eslint-disable-line no-console
    }
    return result;
  } catch (err) {
    ...
    if (emptyFiles && isEmptyFile(filename)) {
      if (emptyFiles === 'warn') {
        console.warn(`Empty settings file: ${filename}`); // eslint-disable-line no-console
      }
      return {};
    }
    throw err;
...
export default {
  ...
  mergeSettings: (
    cwd,
    files,
    { missingFiles = 'warn', emptyFiles = 'warn', loadedFile = 'info' } = {},
  ) => async serverless => {
    // Flag to disable logs has been set, override provided options
    const disableLogs = serverless.variables.options.logs || false;
    if (disableLogs) {
    ...
    let stage = serverless.variables.options.s || serverless.variables.options.stage || undefined;
    if (!stage) {
      const userName = process.env.USER;
      if (!disableLogs) {
        console.warn(chalk.yellowBright(`No stage provided. Using username '${userName}' as stage`)); // eslint-disable-line no-console
      }
      stage = userName;
      serverless.variables.options.stage = stage;
    }
    const loadFile = newFileLoader(serverless, { missingFiles, emptyFiles, loadedFile });
    ...
    // Whether the target region is us-east-1 or not, we still need access to CloudFormation
    // outputs in us-east-1 that were deployed with globalDeployment (true).
    // (Such as referencing edge-lambda outputs from post-deployment deployable units!)
    // (Ab)use the "additional region" setting to pull them in.
    ...
    if (mergedSettingsObj.crossRegionCloudFormation) {
      ...
      const crossRegionSettings = await getCloudFormationCrossRegionValues(
        ...
        additionalAwsRegion,
        mergedSettingsObj,
        mergedSettingsObj.crossRegionCloudFormation,
      );
      Object.assign(mergedSettingsObj, crossRegionSettings);
    }

    /**
     * some of settings needed by add-ons may be absent in helloworld/forks main config.
     * this should allow to enforce an addition of these settings (per SDU) in a user-friendly way.
     *
     * ```
     * # <SDU>/config/.defaults.yml
     * requiredSettings:
     *     myNewSetting: "Please add me!"
     * ```
     */
    if (mergedSettingsObj.requiredSettings) {
      Object.entries(mergedSettingsObj.requiredSettings).forEach(([key, val]) => {
        if (mergedSettingsObj[key] === undefined) {
          throw new Error(val);
        }
      });
    }
