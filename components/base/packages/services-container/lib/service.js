...
class Service {
   ...
   * @param { { inject: boolean } } options if the inject flag inject is set, the services will be injected into the instance
   */
  async service(nameOrNames, { inject } = {}) {
    if (!this.initializationPromise)
    ...
    if (inject) {
      _.concat(nameOrNames).forEach((name, idx) => {
        this[name] = result[idx];
      });
    }
