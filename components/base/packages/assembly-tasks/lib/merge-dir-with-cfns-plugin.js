...
import { mergeCfnYamls } from './merge-cfn-yamls';

function getMergeCfnYmlsTaskPlugin(srcDir) {
  ...
  const getTasks = async (existingTasks, assemblyInfo, slsPlugin, pluginRegistry) => {
    ...
    const tasks = [
      ...
      await mergeCfnYamls({
        from: { dir: parentDir, pattern: `${dirName}/**/*.yml` },
...
export { getMergeCfnYmlsTaskPlugin };
