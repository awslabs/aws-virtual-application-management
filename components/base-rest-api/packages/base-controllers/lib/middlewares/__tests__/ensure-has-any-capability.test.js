...
import controllerFactory from '../ensure-has-any-capability';

const request = {};

describe('ensure-has-any-capability', () => {
  let context;
  ...
  describe('ensures that the user has at least one capability from the specified list of capabilities', () => {
    itProp(
      'throws unauthorized when user does not have any of the required capabilities',
    ...
    itProp(
      `let's the call go through if the user has any of the required capabilities`,
      [
        fc.string().filter(s => !!s),
        fc.constantFrom(
          ['canPerformD', 'canPerformE', 'canPerformA'],
          ['canPerformD', 'canPerformB', 'canPerformC'],
