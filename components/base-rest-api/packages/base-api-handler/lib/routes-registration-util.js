 ...
 * @param {getPlugins} pluginRegistry A registry that provides plugins registered by various components for the specified extension point.
...
async function registerRoutes(context, router, pluginRegistry) {
  ...
  const routesMap = await _.reduce(
    plugins,
    async (routesSoFarPromise, plugin) => plugin.getRoutes(await routesSoFarPromise, pluginRegistry, context),
