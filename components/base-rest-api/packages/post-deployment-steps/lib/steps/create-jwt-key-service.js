  ...
  async cleanup() {
    ...
    try {
      await ssm
        ...
        .promise();
      this.log.info(`Deleted JWT signing from parameter store from path "${paramStoreJwtSecretName}"`);
    } catch (e) {
      if (e.code === 'ParameterNotFound') {
        this.log.info(
          `JWT signing does not exist in parameter store at path "${paramStoreJwtSecretName}"; nothing to do.`,
        );
      }
    }
  }
...
export default CreateJwtKeyService;
