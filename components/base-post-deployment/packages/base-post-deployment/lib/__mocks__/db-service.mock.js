...
class DeleteMock extends HelperBase {
  condition() {
    return this;
  }

  async delete() {
    this.dbService.deleteItem(this.tableName, this.keyValue, this.itemToUpdate);
  }
}
...
class DbServiceMock extends Service {
  async init() {
    ...
    this.helper = {
      ...
      deleter: () => new DeleteMock(this),
    };
  }

  putItem(table, key, item) {
    this._ensureTable(table);
    this.database[table][JSON.stringify(key)] = _.merge(item, key);
  }

  deleteItem(table, key, item) {
    this._ensureTable(table);
    this.deletedItems[table][JSON.stringify(key)] = _.merge(item, key);
    this.database[table][JSON.stringify(key)] = undefined;
  }
  ...
  getDeletedItem(table, key) {
    this._ensureTable(table);
    return this.deletedItems[table][JSON.stringify(key)];
  }
  ...
  _ensureTable(table) {
    ...
    if (!this.deletedItems[table]) {
      this.deletedItems[table] = {};
    }
  }
}
