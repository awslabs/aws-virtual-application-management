...
async function postInit(payload, appContext) {
  ...
  const { appstreamImagesStore, appstreamApplicationsStore, appstreamFleetsStore, dynamicCatalogsStore } = appContext;

  // TODO: Load these stores as needed instead of on startup
  await Promise.all([
    appstreamImagesStore.load(),
    appstreamApplicationsStore.load(),
    appstreamFleetsStore.load(),
    dynamicCatalogsStore.load(),
  ]);
}
