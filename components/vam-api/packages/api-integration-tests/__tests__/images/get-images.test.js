...
describe('Get appstream-images scenarios', () => {
  let setup;
  let adminSession;
  ...
  describe('Get image list', () => {
    it('should fail for anonymous user', async () => {
      ...
      await expect(anonymousSession.resources.images.get()).rejects.toMatchObject({
    ...
    it('should fail for guest user', async () => {
      ...
      await expect(guestSession.resources.images.get()).rejects.toMatchObject({
    ...
    it('should fail for power user', async () => {
      ...
      await expect(powerUserSession.resources.images.get()).rejects.toMatchObject({
        code: errorCode.http.code.forbidden,
      });
    });

    it('should succeed for admin user', async () => {
      await expect(adminSession.resources.images.get()).resolves.toBeDefined();
    });
  });
});
