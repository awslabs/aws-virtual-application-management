...
describe('Get dynamic-catalog scenarios', () => {
  let setup;
  let adminSession;
  ...
  describe('Get dynamic-catalog list', () => {
    it('should fail for anonymous user', async () => {
      const anonymousSession = await setup.createAnonymousSession();

      await expect(anonymousSession.resources.dynamicCatalogs.get()).rejects.toMatchObject({
    ...
    it('should fail for guest user', async () => {
      ...
      await expect(guestSession.resources.dynamicCatalogs.get()).rejects.toMatchObject({
    ...
    it('should succeed for power user', async () => {
      ...
      await expect(powerUserSession.resources.dynamicCatalogs.get()).resolves.toBeDefined();
    });

    it('should succeed for admin user', async () => {
      await expect(adminSession.resources.dynamicCatalogs.get()).resolves.toBeDefined();
    });
  });
});
