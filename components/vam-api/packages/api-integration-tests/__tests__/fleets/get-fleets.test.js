...
describe('Get appstream-fleets scenarios', () => {
  let setup;
  let adminSession;
  ...
  describe('Get fleet list', () => {
    it('should fail for anonymous user', async () => {
      const anonymousSession = await setup.createAnonymousSession();

      await expect(anonymousSession.resources.fleets.get()).rejects.toMatchObject({
    ...
    it('should fail for guest user', async () => {
      ...
      await expect(guestSession.resources.fleets.get()).rejects.toMatchObject({
    ...
    it('should succeed for power user', async () => {
      ...
      await expect(powerUserSession.resources.fleets.get()).resolves.toBeDefined();
    });

    it('should succeed for admin user', async () => {
      await expect(adminSession.resources.fleets.get()).resolves.toBeDefined();
    });
  });
});
